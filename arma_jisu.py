...
class EGARCHX(EGARCH):
    ...
    γ < 0 → leverage (bad news raises vol more than good)
    δ > 0 → liquidity channel (wider spread predicts higher vol)
    """
...
def forecast_one_step(res, scale=1000.0):
    ...
    Uses the ARX mean equation: E[RV_{t+1}] = Const + φ·RV_t
    This is stable, non-simulated, and works for GARCH, GJR-GARCH, and EGARCH-X.

    The GARCH/EGARCH-X variance σ²_{t+1} tells us the UNCERTAINTY of the forecast,
    not the level. We report both the point forecast and ±1σ bounds.
...
def qlike(pred, actual):
    ...
    variance forecasting. Do NOT use the squared form (log(pred²) + actual²/pred²)
...
def garch_is_stationary(res):
    """Returns True if α + β < 0.999 (prevents IGARCH blow-up)."""
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
#  Evaluate a single uploaded CSV against all models
#  (GARCH, GJR-GARCH, EGARCH-X) — produces QLIKE + MSE regardless of regime
# ══════════════════════════════════════════════════════════════════════════════
...
for STOCK_ID in selected_ids:
    ...
    # Step 1: Initial GARCH fit (with t-distribution from global tuning)
    ...
    # Step 2: Residual diagnostics on a sample
    ...
    # Step 3: Choose upgraded model based on diagnostics
    ...
    # Step 4: Collect fitted models and info
    garch_models = {}
    garch_info   = []
    ...
    # Step 5: One-step-ahead forecasts (NO Monte Carlo)
    ...
    # Step 6: Evaluate with full time_id tracking
    ...
    if not run_egarchx:
        ...
        # Direct one-step-ahead forecasts — NO Monte Carlo
